...
// Smart fetch: returns cached data immediately, refetches in background if stale
const cachedFetch = async (url, key, { onData, onBackground } = {}) => {
  const cached = cacheGet(key);
  if (cached) {
    onData?.(cached.data); // instant response
    if (!cached.stale) return; // fresh — skip network
    // stale: refetch silently in background
  }
  try {
...
function SwipeableItem({ item, onDelete, onSend }) {
  ...
  const handleTouchStart = (e) => { startX.current=e.touches[0].clientX; startY.current=e.touches[0].clientY; isLocked.current=false; setSwiping(false); };
  const handleTouchMove  = (e) => {
    ...
    e.preventDefault(); setSwiping(true);
    setOffsetX(Math.min(0,Math.max(-160,dx)));
  };
  const handleTouchEnd = () => {
    if (isLocked.current!=='h') { setOffsetX(0); setSwiping(false); return; }
    if (offsetX<-THRESHOLD) { setOffsetX(-400); setDismissed(true); setTimeout(()=>onDelete(item.id),320); }
    else { setOffsetX(0); setSwiping(false); }
  };
      ...
      <div className="modal-content" style={{ maxWidth:360 }} onClick={(e)=>e.stopPropagation()}>
        ...
        <div style={{ display:'flex',flexDirection:'column',gap:8,marginTop:16 }}>
          <input type="text" placeholder="π.χ. Ψώνια Σαββατοκύριακου" value={value}
            onChange={(e)=>onChange(e.target.value)} onKeyDown={(e)=>e.key==='Enter'&&value.trim()&&onConfirm()} autoFocus
...
export default function App() {
  useKeepAlive(); // 🔑 prevents cold start
  ...
  // ── Offline detection ──────────────────────────────────────────────────────
  useEffect(() => {
    const offlineH = () => { setIsOnline(false); setWasOffline(true); if(navigator.vibrate) navigator.vibrate([200,100,200]); };
    const onlineH  = () => {
      setIsOnline(true);
    ...
    const checkStatus = async () => {
      try {
        ...
        if (r.ok) {
          ...
          // If response took >3s, server was likely waking up
  ...
  const saveCurrentList = () => {
    ...
    setNameModalValue('Ψώνια'); setNameModalOpen(true);
  };
  ...
  const handleMassClear = () => {
    setConfirmModal({
      ...
      onConfirm: () => { setItems([]); setConfirmModal({ open:false,message:'',onConfirm:null }); if(navigator.vibrate) navigator.vibrate(50); },
    });
  };
  ...
  const triggerSearch = async (query, store) => {
    ...
    if (cached) {
      setSuggestions(cached.data.slice(0,30)); // instant from cache
      if (!cached.stale) return;
      // stale: silently refetch
    } else {
      setIsSearching(true); // show skeleton only if no cache
    }
  ...
  const addRecipeToList = async (recipe) => {
    ...
    // Batch in groups of 3 for parallel speed
    const batchSize = 3;
      ...
      {showShareModal && (
        <div className="share-modal-overlay" onClick={()=>setShowShareModal(false)}>
          <div className="share-modal-box" onClick={(e)=>e.stopPropagation()}>
            ...
            <h3>Κοινό Καλάθι</h3>
            <p>Βάλε το <strong>Invite Code</strong> του φίλου σου.</p>
            ...
            <button className="share-connect-btn" onClick={()=>{ if(targetFriendKey.trim()) setShowShareModal(false); }}>✓ Σύνδεση</button>
        ...
        {activeTab==='list' && (
          <div className="tab-content list-tab">
            ...
            <div className="smart-search-wrapper">
              ...
              {/* Skeleton while loading (no cache hit) */}
          ...
          <div className="tab-content brochures-tab">
            ...
            <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill, minmax(140px, 1fr))',gap:12 }}>
              {Object.entries(BROCHURE_LINKS).map(([name,url])=>(
                ...
                >
                  <img src={SUPERMARKET_LOGOS[name]||BROCHURE_LINKS[name]} alt={name}
